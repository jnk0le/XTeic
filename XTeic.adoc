...
Offers less registers/instructions and lacks TMU so it's
...
Example handler implements only jumptable but it can be easily converted into pointer table.
...
consisting of large collection of drivers for external devices, higher portability 
...
NOTE: Both mothods are insufficient for "annotating" unmodified stack arguments 
which are caller saved (documented on arm and defacto on risc-v)
...
more harm than halting until watchdog performs system wide reset.
