...
NOTE: compilers usally don't care about non-abi (interrupt) prologues/epilogues
...
- trump the general IPRA optimizations
...
//??? not suitable for sync with architectural state change??? (ie enable instructions, change `JVT`, `mtvec` etc.)
//??? another bit for CSR sync???

NOTE: equivalent to ARM `isb` intruction
...
The rationale is that the `fence.i` encodes whole instruction synchronization with all zero immediate.
Therefore we can remove all of the sync mechanism by inverting the bits, other than the one designated for
pipeline flush.

The uppermost 5 bits remain zero to allow enabling extra features not covered by `fence.i`.
...
Why would you want to have bg endian loads/stores? +
Probably for handling tasks with "network byte order" which uses big endian representation.

Nice.
So, lets add a big-endian mode (making it configurable at runtime of course), and enjoy 
endian neutral loads/stores (<<cryptogamsaesneutralloads>>) used by networking libraries because one 
cannot be sure on which endianess the code will be run.
