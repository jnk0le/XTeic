...
- to avoid regressions compilers need some kind of heurestic to detect when stacking
...
- very important point is resolving IPRA allocation of callbacks
-- callbacks passed as parameters
-- callbacks initialized once at startup
...
proposed by original author of llvm IPRA iplementation

`\\__attribute__\((regmask("clobbered list here")))`
