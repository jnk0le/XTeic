...
must be 64 byte aligned minimum

implementations might but a further alignment restriction as to avoid 
adder circuits (alignment equal to the next power of 2 of implemented vector entries)
...
| 3 | `nest4` | WLRL | 0 | enable fourth nesting level when cleared
| 2 | `nest3` | WARL | 0 | enable third nesting level when cleared
| 1 | `nest2` | WLRL | 0 | enable second nesting level when cleared
| 0 | `nest1` | WARL | 0 | enable first nesting level when cleared
|====

NOTE: only `nest2` and `nest4` are mandatory
