...
and select the one with highest configured priority, ties are resolved by 
highest irq number.
...
`TEIC_MMIO_CTRL_BASE` + 0x4 // pending rw1c
`TEIC_MMIO_CTRL_BASE` + 0x8 // request set w1s

// not byte addressable

==== pending

`TEIC_MMIO_CTRL_BASE` + 0x20

// 32 * 4

==== priority config

`TEIC_MMIO_CTRL_BASE` + 0x400
...
NOTE: multi hart or when peripheral interrupt lines are shared across multiple cores
