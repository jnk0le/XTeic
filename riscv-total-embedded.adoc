...
The official risc-v debug spec <<riscvdebug>> is good enough to not necessitate another incompatible one.

Biggest offenders are of course the actual implementations that despite beeing the "minimal"
ones designated as "8bit killers", are happily implementing more than necessary.
Like 8-word `progbuf` in ch32v003 <<qingkev2>>.

The very minor things that could "improve" minimal implementations:
...
- no abstract register reads (data exchange only through `datax` message registers)
...
proprietary wch 2 wire and 1 wire
