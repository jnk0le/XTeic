...
- registers must be must be sorted (integer, floating point, vector, custom, then by lowest numbered)
...
NOTE: unannotated `ra` is assumed as a valid return address, `sp` as a valid stack
