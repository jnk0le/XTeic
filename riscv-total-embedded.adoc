...
|====
| range | registers | allocated stack area | mandatory supported | mandatory configurable
| 0 | "x1,x10,x11,reserved" | XLEN * 4 | ✓ | x
| 1 | "x12-x15" | XLEN * 4 | ✓ | x
| 2 | "x4-x7" | XLEN * 4 | x | ✓
| 3 | "x16,x17,x28-x31" | XLEN * 6 | x | ✓
|====
...
NOTE: unlike Zcmp registers are pushed starting from highest stack position
...
higher number in nesting priority or regular priority encodes a higher priority
// ???
...
| 26 | `double_exc_unrec` | ro | 0 | double exception fault of the same or different type
| [25:10] | reserved | rw1c | 0 | reserved
