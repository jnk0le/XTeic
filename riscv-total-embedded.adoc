...
- late arrival handled through mnxti read (possible subtractive jitter of isr entry time)
...
- `entry + 6` cycles of jitter in "inline" handlers.
- `entry + 7 + 16` cycles of jitter in "C-ABI" trampoline entry
- `4` or `abs(entry - 7)` cycles of jitter in  "C-ABI" trampoline epilogue
...
emb-riscv <<embriscv>> is clean sheet design
that attempts to be universal solution for every microcontroller
...
FPU <<spruhs1c>> is more RISC-ish with a bit of VLIW principle in some instructions.
...
NOTE: ISR entry latency is 10 cycles due to 8 stage pipeline.
