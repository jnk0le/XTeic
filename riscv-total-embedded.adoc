...
[cols="1,2,1,1,6",options=header]
|====
| bit | name | type | reset value | description 
| [31:5] | `vect_offset` | WLRL | `TEIC_VECT_BASE>>5` | must be aligned to 64 bytes or rounded up 
													 to next power of 2, whichever is greater
| [4:0] | reserved | WLRL | 0 | reserved
|====

NOTE: alignment reqirement allows to avoid the additional adder circuit during irq dispatch

NOTE: implementations might want to limit the `vect_offset` combinations to existing executable
areas as to reduce necessary state to implement.

NOTE: `vect_offset` can be implemented as a read only constant pointing to beggining of the flash memory
