...
NOTW: Even the PendSV is done by setting `ICSR->PENDSVET` bit instead of executing this mechanism.
...
NOTE: NMIs can still break the "zero jitter" guarantee, thought those should
