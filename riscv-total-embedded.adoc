...
be achieved by regular general purpose architecture (e.g. risc-v) without nesting and WFI loop
...
NOTE: In addition, the `wfi` instruction can optionally stack certain registers ahead of the interrupts,
to reduce their latency. In this case, `sp` is not changed until interrupt arrives.
...
NOTE: Extension designated for smallest devices where a vector table size has a more 
...
and resort to compiler tricks to keep handlers within within 64/128KiB range.
