...
|====
| offset from `TEIC_MMIO_CTRL_BASE` | total bytes | name | byte/half access | description
| 0x0 | 4 | `teic_??` | no |
| 0x4 | 4 | `teic_reset_req??` | no |
| 0x8 | 4 | `teic_Deffered_pending` | no |
| 0xC | 4 | `teic_Deffered_request` | no |
| 0x20 | 32 | `teic_irq_pending` | no |
| 0x40 | 32 | `teicMP_irq_enable` | no | added with <<XteicMP>>
| 0x400 | 1023 | `teic_prio_cfg` | yes |
|====
