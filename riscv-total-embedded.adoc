...
[cols="1,2,4,7",options=header]
|===========================================================================================
| irq number | type | name | notes
| 0 | reserved | - | reserved for startup code (typically jump instruction)
| 1 | NMI | reserved | (optional) 
| 2 | NMI | IntegrityViolation_NMI | (optional) ECC/parity/lockstep on memory and busses
| 3 | NMI | ClockSecurity_NMI | (optional) 
| 4 | NMI | WatchdogViolation_NMI | (optional) aka "early wakeup". Entered right before any
									of the watchdogs trips and performs a (device) reset. 
									Designated for safety measures and error logging, not to reset 
									the watchdog. It shall be assumed that execution could be frozen at 
									this point and no further action can or need to be performed.
| 5 | NMI | MemoryViolation_NMI | Memory access fault or unallowed access to protected area.
								  Covers also istruction fetch from non existing memory region. 
...
| 9 | IRQ | | reserved for ecall???
| 10 | IRQ | | reserved for systick ???
