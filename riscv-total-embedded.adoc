...
when peripheral asserts
becomes pending // at configured nesting priority
if current level is lower than

stacks configured register ranges and updates `sp`
puts content of interrupted `pc` into `ra` register with lowest bit set
sets `in_nestx` in `teic_irq_status`

fetches from vector at `teic_irq_vect`
jumps to the address fetched from vector table
