...
(`fcsr`), and (optionally) multi cycle instruction progress. The content read as 
...
expected that the smallest implementations will not make use of `estate` register.
...
| 10 | `in_nmi_lockup` | ro | 0 | readable only by debugger
								  can be cleared only by hart/system reset
| 9 | `in_nmi_unrecoverable` | ro | 0 | can be cleared only by hart/system reset
| 8 | `in_nmi` | ro | 0 | 
| 7 | `in_nest4` | ro | 0 | 
| 6 | `in_nest3` | ro | 0 | 
| 5 | `in_nest2` | ro | 0 | 
| 4 | `in_nest1` | ro | 0 | 
| 3 | `pending_nest4` | ro | 0 | 
| 2 | `pending_nest3` | ro | 0 | 
| 1 | `pending_nest2` | ro | 0 | 
| 0 | `pending_nest1` | ro | 0 |
|====
...
enter the interrupt at the highest nesting level
always within the same 
regardless of the interrupted execution state
(be it WFI, execution from waitstated memory, entry/exit of lower level interrupts)
...
It shall be also assumed that the interrupts at the highest nesting level will not be
masked out and no HW measueres will be present to ??
...
a lot of extras to implement
probe/write peripherals directly

=== no nmi/exception nesting

in normal scenario such condition is very rare
easy way to overflow the stack

there is no MMU

NMI is a fatal condition anyway

=== no triggering interrupt handlers by software

aka software trigger 

usually for unallocated vectors

redundant
complexity
requires mechanism clearig pendingpending

implement dedicated software deffered interrupts instead

rarely used

=== no stack realignment upon interrupt entry/exit

This is just a waste of hardware

The ABI mandated to 8 bytes
...
`mtval` is often not impelemnted anyway, even by uarch without unaligned loads/stores
...
NOTE: There was even an CVE related to uninitialized `mtvec`: https://youtu.be/iz_Y1lOtX08?t=1740
...
This also allows us to use vector address with zeroed lowest bits. 
