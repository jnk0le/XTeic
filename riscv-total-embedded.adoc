...
on custom patches. Naked handler + mret trick doesn't work in llvm, it shoud break in gcc anyway due to
...
must use raw register name instead of ABI mnemonic
to be used in ABI agnostic way
ie.
used on different ABIs without changing 
...
embedded scenario of main or thread functions with an infinite loop.
