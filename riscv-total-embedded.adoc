...
However, certain hardened cores need to have all registers initialized to consistent state, as to
avoid integrity faults when stacking out yet unused registers.
In most cases, it's still possible to require initialization of all registers in startup code.
