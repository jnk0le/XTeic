...
requires wasting more codepoints by `push`/`pop` instructions.
...
return mechanism to exit iterrupt context

Even worse there are already hardware stackers designed for ilp32e. When the new and better ABI will be 
introduced, it will be impossible to use with pre-existing HW stackers.

Therefore we need universal way to annotate 

`\\__attribute__\((prestacked("")))`

use raw register name instead of ABI mnemonic
to make portable across ABIs
to be used in ABI agnostic way
...
It doesn't work on prologues though, leading to waste of stack and codespace in the most typical
...
By abusing the "prestacked annotation" we can get rid of this prologue by "prestacking" of all available registers. +
e.g. `\\__attribute__\((prestacked("x1,x5-x31,f0-f31,fcsr")))`
