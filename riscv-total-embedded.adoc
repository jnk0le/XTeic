...
| TEIC_VECT_BASE | implementation specific | Base address of the application entry point 
											 as well as its the vector table. May have additional
...
==== startup behaviour
 ...
 //up to the bootloader to redirect?? `teic_irq_vect` to TEIC_VECT_BASE
...
`TEIC_VECT_BASE` implementation constant

The `TEIC_VECT_BASE` must be 64KiB aligned.
