...
[cols="2,1,3,1,1,4",options=header]
|====
| offset from `TEIC_MMIO_CTRL_BASE` | total bytes | name | byte/half access | type | description
| 0x0 | 4 | `teic_??` | no | |
| 0x4 | 4 | `teic_reset_req??` | no | rw?? |
| 0x8 | 4 | `teic_Deffered_pending` | no | rw1c |
| 0xC | 4 | `teic_Deffered_request` | no | w1s |
| 0x20 | 32 | `teic_irq_pending` | no | ro |
| 0x40 | 32 | `teicMP_irq_enable` | no | rw | added with <<XteicMP>>
| 0x400 | 1023 | `teic_prio_cfg` | yes | rw |
|====
...
|====
| bit | name | type | reset value | description 
| [31:1] | `deffered[i]_req` | w1s | 0 | (optional) write 1 to send deffered1-deffered31 irq requests
| [0] | `deffered0_req` | w1s | 0 | write 1 to send deffered0 irq request
|====
...
consists of 1023 entries 
first 8 entries (NMIs) are reserved

1 byte  `teic_prio_cfg[IRQn]`

===== priority encoding

each implemented irq vector 
...
|====
| bit | name | type | reset value | description 
| [8:(8-`TEIC_IRQ_NESTING_BITS`)] | `nest_prio` | rw | 0 | nesting priority bits
| [(8-`TEIC_IRQ_NESTING_BITS`):(8-(`TEIC_IRQ_NESTING_BITS`+`TEIC_IRQ_PRIORITY_BITS`))] | `sub_prio` | rw | 0 | sub-priority bits
| [(8-(`TEIC_IRQ_NESTING_BITS`+`TEIC_IRQ_PRIORITY_BITS`)):0] | reserved | rw | 0 | reserved
|====

// nesting prority bits is implementation constant
// top to bottom
// nesting then sub-prio
