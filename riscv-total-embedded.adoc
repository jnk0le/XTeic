...
| 0xbc4 | `nmi_cause` | MRW (RO?) | coarse mask of NMI cause?
| 0xbc5 | `cfg` | MRW | config register
| 0xbc6 | `` | |
|====
...
`fcsr`, or (optionally) multi cycle instruction progress. The content read as 
...
It shall be assumed that highest level interrupt cdoe and stack resides in zero waitstated memories and no HW measures will be implemented to adjust for a different scenario.
|====
...
=== no triggering interrupts by middle??
