...
NOTE: on riscv it's `s0` and `s1`, in presence of Zcmp[e] pushing `s0,s1` is free in non 
leaf functions, and just 2 16bit instructions in leaf.
...
[source, asm]
```
.word (0b0000000<<25)|(8<<20)|(0<<15)|(10<<7)|0x43 
...
NOTE: it is portable to other architectures, the more caller saved registers are available,
the higher relative gain is.
...
in case of Zcmp[e]
in a leaf function handlers it 
more efficient to push s registers 
instead of
